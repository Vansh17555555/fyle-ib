...
def test_get_assignments_in_graded_state_for_each_student():
    ...
    submitted_assignments = Assignment.filter(Assignment.student_id == 1).all()  # Fetch all results
    ...
    expected_result = [(1, len(submitted_assignments))]  # Use len on the list of results
    ...
    # Filter the result to only include entries for student_id 1
    sql_result_filtered = [result for result in sql_result if result[0] == 1]

    assert sql_result_filtered == expected_result
...
def test_get_grade_A_assignments_for_teacher_with_max_grading():
    ...
    # Execute the SQL query and check if the count matches the created assignments
    sql_result = db.session.execute(text(sql)).fetchall()
    assert sql_result[0][0] >= grade_a_count_1  # Check if the count is at least as much as expected
    ...
    # Execute the SQL query again and check if the count matches the newly created assignments
    sql_result = db.session.execute(text(sql)).fetchall()
    assert sql_result[0][0] >= grade_a_count_2  # Check if the count is at least as much as expected
